...
GITHUB_MIRROR="${GITHUB_MIRROR:-https://wget.la}"
...
# Pending Removal
# cubiq is no longer maintaining ComfyUI custom nodes
...
for line in "${array[@]}"; do
    url="$line"
    [[ -n "$GITHUB_MIRROR" && "$url" == https://github.com/* ]] && url="${GITHUB_MIRROR}/${url}"
    curl -w "\n" -sSL "${url}" >> pak5.txt
done
